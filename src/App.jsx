...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: 'system-ui, -apple-system, sans-serif', color: '#1e293b' }}>
      ...
      <div style={{ padding: '10px 20px', backgroundColor: '#0f172a', textAlign: 'right', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
        <button 
          onClick={() => setIsAdminView(!isAdminView)}
          style={{ 
            cursor: 'pointer', 
            padding: '6px 14px', 
            backgroundColor: isAdminView ? '#2563eb' : '#334155', 
            color: 'white', 
            border: 'none', 
            borderRadius: '6px',
            fontWeight: '600',
        ...
        >
          {isAdminView ? "Ver Web del Hotel" : "Ir al Panel Admin"}
        </button>
      </div>

      {isAdminView ? (
        <AdminDashboard bookings={confirmedBookings} setBookings={setConfirmedBookings} />
      ) : (
